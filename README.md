...
### 3、this指向组件实实例   ->  指向组件代理对象
     访问属性 -> vue2组件实例 -> 返回属性值；  
     访问属性 -> 组件代理对象 -> 访问属性 -> 组件实例 -> 返回属性值 -> 组件代理对象 -> 返回属性值

### 4、生命周期

### 5、看vue2迁移vue3文档
      （1）升级vue-cli; 检查所用的库有没有升级对应vue3版（element-plus）
      （2）ref属性获取节点对象
     ...
     （4）v-bind.sync替换为v-model
          :visible.sync="showModify"  -> v-model="showModify"
     （5）删除filters, 建议使用方法调用或计算属性替代
     （6）删除$children, 建议使用上条ref属性代替
     （7）native修饰符
